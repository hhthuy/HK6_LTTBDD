    ...
    <body>
        ...
        <table style="border: 1px solid; width: 100%; text-align:center">
            <c:url var="sortLinkFirstName" value="/person/list">
                <c:param name="sortField" value="<%= Integer.toString(SortUtils.FIRST_NAME)%>" />
                <c:param name="sortUD" value="${sortUD}" />
                <c:param name="lastSortField" value="${sortField}" />
            </c:url>
            <c:url var="sortLinkLastName" value="/person/list">
                <c:param name="sortField" value="<%= Integer.toString(SortUtils.LAST_NAME)%>" />
                <c:param name="sortUD" value="${sortUD}" />
                <c:param name="lastSortField" value="${sortField}" />
            </c:url>
            <c:url var="sortLinkMoney" value="/person/list">
                <c:param name="sortField" value="<%= Integer.toString(SortUtils.MONEY)%>" />
                <c:param name="sortUD" value="${sortUD}" />
                <c:param name="lastSortField" value="${sortField}" />
            </c:url>
